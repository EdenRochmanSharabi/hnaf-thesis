...
"""
    ...
    def test_custom_functions(self):
        ...
        try:
            ...
            # Construir matrices (3D)
            A1 = np.array([[float(self.a1_vars[i][j].get()) for j in range(3)] for i in range(3)])
            A2 = np.array([[float(self.a2_vars[i][j].get()) for j in range(3)] for i in range(3)])
            
            # Probar transformaciones (3D)
            coord_vector = np.array([[x0], [y0], [0.0]])  # Agregar coordenada z para 3D
            x1 = A1 @ coord_vector
            x2 = A2 @ coord_vector
    ...
    def save_custom_functions(self):
        ...
        try:
            ...
            # Actualizar matrices (3D)
            A1 = [[float(self.a1_vars[i][j].get()) for j in range(3)] for i in range(3)]
            A2 = [[float(self.a2_vars[i][j].get()) for j in range(3)] for i in range(3)]
    ...
    def get_training_parameters(self):
        ...
        try:
            # Construir matrices A1 y A2 desde los campos de la interfaz (3D)
            A1 = [[float(self.a1_vars[i][j].get()) for j in range(3)] for i in range(3)]
            A2 = [[float(self.a2_vars[i][j].get()) for j in range(3)] for i in range(3)]
            
            # DEBUG: Verificar matrices que se están enviando
            print(f"🔍 DEBUG - Matrices desde GUI:")
            print(f"   A1: {A1}")
            print(f"   A2: {A2}")
            print(f"   A1 shape: {len(A1)}x{len(A1[0])}")
            print(f"   A2 shape: {len(A2)}x{len(A2[0])}")
